...
shared.fanClient().then(client => {
    ...
    .then(fg => console.log('We have joined the fangroup. ActionStatus = %s', sdk.fan.FAN_GROUP_ACTION_STATUS[fg.actionStatus]))
    // leave it again
    .then(() => client.fanService.fanGroupService.leaveFanGroup(fgId))
    .then(fg => console.log('We have left the fangroup. ActionStatus = %s', sdk.fan.FAN_GROUP_ACTION_STATUS[fg.actionStatus]));
